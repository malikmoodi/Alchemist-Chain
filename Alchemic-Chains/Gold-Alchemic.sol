    ...
    UUPSUpgradeable
{
    ...
    mapping(address => userGoldChainList) private userChainList;
    mapping(uint256 => chainDetail) private chainDetails;

    //mapping (address => bool )
    ///////////////////////////// Error variables //////////////////

    string constant NOT_AUTHORIZED = "GCS1";
    string constant WALLET_LIMIT_REACHED = "GCS2";
    string constant PAUSED = "GCS3";
    string constant NOT_GCM = "GCS4";
    /////////////////////////////////////////
    constructor() {
    ...
    */
    function initialize(address _settingContract, address _timer, address _walletSupervisor) public initializer {
        __ERC721_init("ALCHEMIST CHAINS - GOLD", "ALCC-G");
        __Pausable_init();
        __Ownable_init();
        ...
        rewardsPerPeriod = 1*10**18;
        rewardPeriod = 5 minutes;
        perWalletLimit = 5;
    }

    /**
    * @dev createChain: Only Gold chain manager can call this method. Saves detail of Chain against user address and chain token Id. 
    *@param  to Address of user 
    *@param  name Name of chain
    *
    */
    function createChain(address to, string memory name)
        public
        returns (uint256 _tokenId)
    {
        require(goldChainManager == msg.sender, NOT_GCM);
    ...
    */
    ...
    function claimRewardsToken(uint256 tokenId, uint256 amount)
        public
    {
        require(
            _exists(tokenId),
    ...
    */
    function calculateRewardsToken(uint256 tokenId)
        public
        view
        returns (uint256 rewards)
    {
        ...
        uint256 timeSinceClaim = (ITimer(timerContract).getDay().sub(detail.lastClaimTime));
            // .div(rewardPeriod);
    ...
    */
    function burn(uint256 tokenId, address owner) public {
        require(ownerOf(tokenId) == owner || goldChainManager == msg.sender, NOT_AUTHORIZED);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize)internal override{
        if(paused())
        {
        	if( msg.sender == goldChainManager || msg.sender == owner()){
    ...
    */ 
    function getUserDetails(address user)
        public
        view
        returns (userGoldChainList memory)
    {
    ...
    */ 
    function getChainDetails(uint256 tokenId)
        public
        view
        returns (chainDetail memory)
    {
    ...
    */ 
    function getUserChains(address user)   public view returns (uint256[] memory chains){
    ...
    */
    function setRewardPerPeriod(uint256 reward) public onlyOwner {
